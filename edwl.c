...
static void mvwlr(Client *c, int floating, int step);
...
const char edwl_version[] = "edwl 0.9.5";
char *status_text = NULL;
cairo_surface_t *background_image_surface = NULL;
unsigned int background_image_path_hash = 0;
...
void mouseclick(int button, int mod) // mod - true, if MODKEY pressed
{
  ...
  if (selc && selc->isfullscreen || active_constraint)
    return;

  /* If there is a client under cursor -> don't process */
  ...
  if (local_y > (selmon->m.height - barheight - margin_bar) && !c)
  {
    ...
    if (local_x > margin_bar && local_x < (margin_bar + barheight))
    {
        Client *c;
        wl_list_for_each(c, &clients, link)
        {
...
createmon(struct wl_listener *listener, void *data)
{
  ...
  wlr_output_enable_adaptive_sync(wlr_output, r == END(monrules) ? 1 : r->adaptive_sync);
...
createpointerconstraint(struct wl_listener *listener, void *data)
{
  if (selclient()) {
     ...
     PointerConstraint *pointer_constraint = calloc(1, sizeof(PointerConstraint));
...
setfullscreen(Client *c, int fullscreen)
{
  ...
  if (fullscreen) {
    /* If bar is always on top, we better move a client on top here */
    if (baralwaysontop)
      wlr_scene_node_reparent(c->scene, layers[LyrTop]);
    ...
    /* If bar is always on top, we better move a client behind */
    if (baralwaysontop)
      wlr_scene_node_reparent(c->scene, layers[c->isfloating ? LyrFloat : LyrTile]);
...
void
mvwlr(Client *c, int floating, int step)
{
	Monitor *mon = selmon, *m;
	c->isfloating = floating;
	if (floating) {
...
void
mvwud(Client *c, int floating, int step)
{
	Monitor *mon = selmon, *m;
	c->isfloating = floating;
	if (floating) {
...
focusclient(Client *c, int lift)
{
  ...
  wl_list_for_each(current, &m->bar.clients, link)
  {
    if (current->c == c)
    {
      ...
      MOVENODE(m, &m->bar.rects[BarFocusedClientRect]->node, current->box.x + margin_bar, margin_bar);
    }
  }
...
keypress(struct wl_listener *listener, void *data)
{
  ...
  if (Lock.set && event->state == WL_KEYBOARD_KEY_STATE_PRESSED)
  {
    ...
    if (syms[0] == XKB_KEY_Return)
    {
      ...
      if (!strcmp(Lock.password, codetounlock))
        lockdeactivate();
      else
...
initbarrendering(Monitor *m)
{
  ...
  MOVENODE(m, &m->bar.texts[BarTagsText].data->node, margin_bar, margin_bar);
  ...
  for (int i = 0, current_width = 0; i < layouts_amount; i++)
  {
    ...
    MOVENODE(m, &m->bar.layout_signs[i].data->node, m->bar.tags_border + margin_bar, margin_bar);
  ...
  MOVENODE(m, &m->bar.rects[BarSelectedTagRect]->node, margin_bar, margin_bar);
  ...
  for (int i = 0, w = 0, h = 0, w_e; i < tags_amount; i++)
  {
    ...
    MOVENODE(m, &m->bar.tags_info[i].has_clients_rect->node, m->bar.tags_info[i].left + w_e + margin_bar, margin_bar );
    MOVENODE(
      m,
      &m->bar.tags_info[i].has_clients_not_focused_rect->node,
      m->bar.tags_info[i].left + w_e + 1 + margin_bar,
      (1 + margin_bar)
    );
...
togglesticky(const Arg *arg)
{
  ...
  if (sel->issticky)
  {
    sel->backup_tags = sel->tags; 
    sel->tags = sel->tags ^ ~sel->tags;
  } else
  {
    sel->tags = sel->backup_tags; 
    arrange(sel->mon);
  }
}
...
updateclientbounds(Monitor *m)
{
  ...
  {
    clients_amount = 0;
    is_sel_tag = (current_tag_id & TAGMASK) == m->tagset[m->seltags];

    wl_list_for_each(current, &m->bar.clients, link)
    {
      if (m == current->c->mon)
      {
        if (current->c->tags & current_tag_id && !current->c->issticky)
          ++clients_amount;

        if (current->c->issticky && is_sel_tag)
          ++clients_amount;
      }
    }
    ...
    if (clients_amount)
    {
      ...
      /* To no disable BarFocusedClientRect */
      if (is_sel_tag)
        some_clients_on_seltag = 1;
      ...
      wl_list_for_each(current, &m->bar.clients, link)
      {
        ...
        if (m == current->c->mon && current->c->tags & current_tag_id)
        {
          ...
          MOVENODE(m, &current->surface.data->node, current_point + margin_bar, margin_bar);
  ...
  wlr_scene_node_set_enabled(&m->bar.rects[BarFocusedClientRect]->node, some_clients_on_seltag);
}
...
updatebacks(struct wl_listener *listener, void *data)
{
  ...
  struct wlr_fbox back_box = {0, 0, 0, 0};
  unsigned int hash = djb2hash(tray->background_path);

  if (hash == background_image_path_hash)
    return;
  ...
  background_image_path_hash = hash;
}
...
view(const Arg *arg)
{
  ...
  MOVENODE(selmon, &selmon->bar.rects[BarSelectedTagRect]->node, selmon->bar.tags_info[active_tag].left + margin_bar, margin_bar);
  updateclientbounds(selmon);
